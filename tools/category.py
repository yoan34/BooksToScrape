...
# Les différentes en-têtes qui sera placé dans les fichiers CSV.
CSV_HEADERS = [
    'product_page_url',
...
def get_books_from_category(url, category):
    ...
    if response.ok:
        ...
        # Création du dossier CSV qui réception les fichiers CSV si
        # il existe pas.
        ...
        with open(path, 'w', newline='', encoding='utf-8') as file:
            ...
            # Cette condition permet d'ajouter les urls des différentes pages
            # si elles existent, sinon on ajoute simplement l'url de base.
            if number_of_pages:
                number_of_pages = int(number_of_pages.text.split()[-1])
                for page in range(number_of_pages):
                    pages.append('/'.join(url.split('/')[:-1]) +
                                 '/page-{}.html'.format(page+1))
            else:
                pages.append(url)

            # On navigue à travers toutes les urls disponibles dans la liste
            # 'pages'.
            for page in pages:
                ...
                if response.ok:
                    ...
                    # on récupère le lien de chaque livre et leurs informations
                    # souhaitées.
                    for n, book in enumerate(books):
                        ...
                        # Utilise la solution d'encodage et décodage pour
                        # afficher correctement £ et autres caractères dans
                        # le fichier CSV (seul solution trouvé).
                        writer.writerow(
                            [d.encode('raw_unicode_escape').decode('utf-8')
                                for d in data]
                            )
    else:
