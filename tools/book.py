...
def get_book(url, category, n):
    response = requests.get(url)

    if response.ok:
        soup = BeautifulSoup(response.text, 'html.parser')

        # Récupère les données dans la section "Product Information"
        informations = soup.findAll('td')

        # Supprime des données inintéressantes
        [informations.pop(index) for index in [1, 3, 4]]
        ...
        review_rating = NUMERATION_REVIEW_RATING[
                            soup.find(
                                'p',
                                {'class': 'star-rating'},
                                )['class'][1]]
        ...
        get_image(image_url, category, n)
        print('     book: {}'.format(
